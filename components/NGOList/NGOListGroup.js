...
const requestURLs = {
	openings:
		"https://v2.api.atados.com.br/project/list?closed=false&in_review=false&ordering=-published,closed,-published_date&paused=false&published=true&search=design&types=1&limit=5",
...
const NGOListGroup = ({ data }) => {
	...
	return (
		<article className={styles.list}>
			...
			<div dangerouslySetInnerHTML={{ __html: data.ngoListIntro }} />
			{error.includes(data.ngoListType) ? (
				<div className={styles.error}>
					Houve um erro ao acessar os dados. Por favor, acesse diretamente o{" "}
					<a href="https://atados.com.br" target="_blank" rel="noreferrer">
						site da Atados
					</a>{" "}
				...
				<div>
					{list?.map((item) => (
						<Card
							...
							cityState={item.address?.city_state}
						/>
					))}
					<a
